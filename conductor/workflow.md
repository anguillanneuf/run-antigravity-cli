...
6.  **Verify Coverage:** Run coverage reports using the project's chosen tools.
    For our Python project: `pytest --cov=src --cov-report=term-missing` Target: >80% coverage for new code.
...
8.  **Commit Code Changes:**
    ...
    -   Propose a clear, concise commit message e.g, `feat(action): Parse repository event metadata from pull request JSON`.
    -   Perform the commit.
...
11. **Commit Plan Update:**
    ...
    -   **Action:** Commit this change with a descriptive message (e.g.,
        `conductor(plan): Mark task 'Create GitHub API client' as complete`).
...
3.  **Execute Automated Tests with Proactive Debugging:**
    ...
    -   **Example Announcement:** "I will now run the automated test suite to
        verify the phase. **Command:** `pytest`"
