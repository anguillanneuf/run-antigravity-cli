...
@pytest.mark.asyncio
@patch("src.github_client.GitHubEventContext.from_env")
@patch("src.github_client.GitHubClient")
@patch("src.review_engine.AntigravityReviewEngine")
async def test_entrypoint_pull_request_success(
    ...
    with patch("src.entrypoint.parse_diff_to_changed_lines") as mock_parse_diff:
        ...
        mock_client_cls.assert_called_once_with(token="mock-github-token-456", repo="google/run-antigravity-cli")
        mock_client.fetch_pr_diff.assert_called_once_with(pr_number=42)
        ...
        mock_engine.run_review.assert_called_once_with(
            diff_text="mock-diff-patch",
            changed_lines={"src/main.py": [5]}
        )
